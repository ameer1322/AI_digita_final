...
def schema(
    ...
    *,
    ...
    """
    Process a list of model and generate a single JSON Schema with all of them defined in the ``definitions``
    top-level JSON key, including their sub-model.

    :param models: a list of model to include in the generated JSON Schema
    ...
    :return: dict with the JSON Schema with a ``definitions`` top-level key including the schema definitions for
      the model and sub-model passed in ``model``.
    """
...
def model_schema(
    ...
    """
    Generate a JSON Schema for one model. With all the sub-model defined in the ``definitions`` top-level
    JSON key.
    ...
    if model_name in nested_models:
        # model_name is in Nested model, it has circular references
...
def field_schema(
    ...
    *,
    ...
    Also return a dictionary of definitions with model as keys and their schemas as values. If the passed field
    is a model and has sub-model, and those sub-model don't have overrides (as ``title``, ``default``, etc), they
    ...
    :param model_name_map: used to generate the JSON Schema references to other model included in the definitions
...
def get_model_name_map(unique_models: TypeModelSet) -> Dict[TypeModelOrEnum, str]:
    """
    Process a set of model and generate unique names for them to be used as keys in the JSON Schema
    definitions. By default the names are the same as the class name. But if two model in different Python
    modules have the same name (e.g. "users.Model" and "items.Model"), the generated names will be
    based on the Python module path for those conflicting model to prevent name collisions.

    :param unique_models: a Python set of model
    :return: dict mapping model to names
    """
    name_model_map = {}
...
def get_flat_models_from_model(model: Type['BaseModel'], known_models: Optional[TypeModelSet] = None) -> TypeModelSet:
    """
    Take a single ``model`` and generate a set with itself and all the sub-model in the tree. I.e. if you pass
    ...
    :param known_models: used to solve circular references
    :return: a set with the initial model and all its sub-model
    """
    known_models = known_models or set()
...
def get_flat_models_from_field(field: ModelField, known_models: TypeModelSet) -> TypeModelSet:
    ...
    (so, it could be a submodel), and generate a set with its model and all the sub-model in the tree.
    ...
    :param field: a Pydantic ``ModelField``
    :param known_models: used to solve circular references
    :return: a set with the model used in the declaration for this field, if any, and all its sub-model
    """
    from .main import BaseModel
...
def get_flat_models_from_fields(fields: Sequence[ModelField], known_models: TypeModelSet) -> TypeModelSet:
    ...
    (so, any of them could be a submodel), and generate a set with their model and all the sub-model in the tree.
    ...
    :param known_models: used to solve circular references
    :return: a set with any model declared in the fields, and all their sub-model
    """
    flat_models: TypeModelSet = set()
    for field in fields:
...
def get_flat_models_from_models(models: Sequence[Type['BaseModel']]) -> TypeModelSet:
    """
    Take a list of ``model`` and generate a set with them and all their sub-model in their trees. I.e. if you pass
    a list of two model, ``Foo`` and ``Bar``, both subclasses of Pydantic ``BaseModel`` as model, and ``Bar`` has
...
def field_type_schema(
    ...
    *,
    ...
    information as title, etc. Also return additional schema definitions, from sub-model.
    """
...
def model_process_schema(
    ...
    *,
    ...
    Take a single ``model`` and generate its schema. Also return additional schema definitions, from sub-model. The
    sub-model of the returned schema will be referenced, but their definitions will not be included in the schema. All
...
def field_singleton_schema(  # noqa: C901 (ignore complexity)
    ...
    *,
    ...
    Take a single Pydantic ``ModelField``, and return its schema and any additional definitions from sub-model.
    """
    from .main import BaseModel
