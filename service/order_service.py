from fastapi import HTTPException
from typing import Optional, List
from starlette import status

from model.order_model import Order

from model.order_request import OrderRequest
from repository import order_repository

async def get_orders():
    return await order_repository.get_orders()

async def get_order_by_user(user_id:int):
    return await order_repository.get_order_by_user(user_id)

async def check_order_exists(order_id:int):
    return await order_repository.check_order_exists(order_id)

async def add_item_to_order(order : Order, order_id : int):
    return await order_repository.add_item_to_order(order)

async def create_new_order(order:Order,user_id:int):
    return await order_repository.create_new_order(order,user_id)

async def create_order_item(order: Order, order_id: int):
    return await order_repository.create_new_order_item(order, order_id)


async def handle_order(order : OrderRequest, user_id):
    order_id = await get_order_by_user(user_id)
    if order_id:
        check_order_item = await order_repository.check_order_item(order, user_id)
        if check_order_item:
            return add_item_to_order(order, order_id)
        return await order_repository.create_new_order_item(order, order_id)
    order_id = await create_new_order(order,user_id)
    return await order_repository.create_new_order_item(order, order_id)


async def delete_order(order_id:int):
    order_id = await check_order_exists(order_id)
    if order_id:
        await order_repository.delete_order(order_id)
